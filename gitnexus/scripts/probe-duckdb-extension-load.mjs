...
function parseLbugMaxDbSize(raw) {
  ...
  if (!Number.isFinite(parsed) || parsed <= 0) {
    throw new Error(`Invalid LadybugDB max DB size for extension probe: ${raw ?? '<missing>'}`);
  }
  return Math.floor(parsed);
}

async function probeDuckDbExtensionLoad(extensionName) {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'gitnexus-ext-probe-'));
  const dbPath = path.join(tmpDir, 'probe.lbug');
  let db;
  let conn;
...
probeDuckDbExtensionLoad(process.argv[2] ?? process.env.GITNEXUS_LBUG_EXTENSION_NAME).catch(
  (err) => {
